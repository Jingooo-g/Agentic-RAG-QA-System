...
# ==============================================================================
# 1. 检索模块初始化 (Dual Retriever)
# ==============================================================================
try:
    # 导入两个检索器
    from retrieval import VectorSearcher, BGEVectorizer, BM25Retriever
except ImportError:
    print("Error: Could not import classes from 'retrieval.py'.")
    sys.exit(1)

print(">>> Initializing Retrievers...")

# --- A. 初始化 Dense Retriever (BGE) ---
VECTOR_DB_PATH = "data/bge_vector_db.jsonl"
vector_searcher = None
query_encoder = None
doc_text_map = {}

if os.path.exists(VECTOR_DB_PATH):
    print(f"[Dense] Loading Vector DB from {VECTOR_DB_PATH}...")
    ...
    with open(VECTOR_DB_PATH, 'r', encoding='utf-8') as f:
        for line in f:
            try:
                ...
                doc_text_map[item["id"]] = item.get("text", item.get("content", ""))
            except: continue
else:
    print("[Warning] Vector DB not found. Dense retrieval will be unavailable.")

# --- B. 初始化 Sparse Retriever (BM25) ---
# 注意：这会花一点时间构建索引
bm25_retriever_instance = BM25Retriever(local_file="data/collection.jsonl")


# --- C. 定义统一调度器 ---
# generation.py 中更新这个类
...
class RetrieverDispatcher:
    def retrieve(self, query: str, method: str = "dense", top_k: int = 10) -> List[Dict]:
        ...
        # 1. 如果需要 Dense 或 Hybrid，先跑 Dense
        ...
        if method == "hybrid":
            # 简单策略：交替合并或去重合并
            # 这里使用：保留 Dense 的前 50% + Sparse 的前 50%，去除重复
            combined = []
            seen_ids = set()
            ...
            for doc in sparse_results:
                if doc['doc_id'] not in seen_ids:
                    # 为了不破坏分数排序，给 BM25 的分数做一个缩放或者标记（可选）
                    # 这里直接追加，简单粗暴但有效
                    combined.append(doc)
                    seen_ids.add(doc['doc_id'])
            
            # 重新截断到 top_k (保证上下文不超长)
            return combined[:top_k]
            
        return dense_results # 默认回退

# 全局检索器实例
retriever = RetrieverDispatcher()

# ==============================================================================
# 2. RAG 核心逻辑 (保持原有逻辑，仅微调模型参数)
# ==============================================================================

class BaseGenerator:
    """生成器基类（与generation_module.py接口保持一致）"""
...
class EntityProcessor:
    ...
    def extract_main_entity(text: str, answer: Optional[str] = None) -> str:
        if answer and "i don't know" in answer.lower():
            answer = None
        candidates = []
        exclude_words_for_proper_nouns = {"who", "what", "where", "when", "why", "how", "and", "the", "a", "an", "i", "don", "know", "yes", "no", "of", "in", "with", "for", "on", "at", "by", "from", "up", "about", "into", "over", "after", "beneath", "below", "between", "through", "above", "around", "among", "across", "behind", "before", "down", "off", "out", "under", "upon", "within", "without", "throughout", "is", "was", "were", "be", "been", "being", "has", "have", "had", "do", "does", "did", "will", "would", "shall", "should", "can", "could", "may", "might", "must", "are", "this", "that", "these", "those", "he", "she", "it", "they", "we", "you", "me", "him", "her", "us", "them", "my", "your", "his", "her", "its", "our", "their", "to", "as", "but", "or", "nor", "yet", "so", "if", "unless", "until", "while", "when", "where", "why", "how", "than", "then", "once", "though", "although", "even", "because", "since", "before", "after", "while", "whether", "else", "such", "only", "just", "also", "too", "very", "really", "quite", "rather", "almost", "always", "never", "often", "sometimes", "usually", "seldom", "hardly", "scarcely", "barely", "ever", "once", "twice", "thrice", "again", "further", "more", "most", "less", "least", "much", "many", "few", "little", "some", "any", "no", "every", "all", "both", "each", "either", "neither", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine", "ten", "first", "second", "third", "fourth", "fifth", "other", "another", "such", "what", "which", "who", "whom", "whose", "where", "when", "why", "how", "here", "there", "then", "now"}
        extra_excludes = {"additionally", "explanation", "provided", "evidence", "therefore", "insufficient", "based", "solely", "given", "cannot", "explanation:"}
        banned_starters = {"are","is","was","were","do","does","did","can","could","would","should","which","what","who","whom","whose","where","when","why","how"}
        ...
        if answer:
            ...
            for m in proper_ans:
                token = EntityProcessor.clean(m)
                if token.lower() not in exclude_words_for_proper_nouns and token.lower() not in extra_excludes and len(token) > 2:
                    candidates.append(token)
        ...
        for m in proper:
            m2 = re.sub(r'^(Are|Is|Do|Does|Did|Can|Could|Would|Should|Which|What|Who|Whom|Whose|Where|When|Why|How)\s+', '', m)
        ...
        if answer:
            answer_candidates = [EntityProcessor.clean(m) for m in re.findall(r'([A-Z][a-zA-Z]+(?:\s[A-Z][a-zA-Z]+)*)', answer)
                                 if EntityProcessor.clean(m).lower() not in exclude_words_for_proper_nouns and EntityProcessor.clean(m).lower() not in extra_excludes and len(EntityProcessor.clean(m)) > 2]
            answer_candidates = [c for c in answer_candidates if not re.match(r'^(?:' + '|'.join(banned_starters) + r')\b', c.lower())]
    ...
    def extract_entities(text: str) -> List[str]:
        exclude = {"who","what","which","where","when","why","how","and","the","a","an","are","is","was","were","do","does","did","can","could","would","should","of","in","on","at","by","from","into","over","after","beneath","below","between","through","above","around","among","across","behind","before","down","off","out","under","upon","within","without","throughout","to","as","but","or","nor","yet","so","if","unless","until","while","than","then","once","though","although","even","because","since","whether","else","only","just","also","very","both","either","vs","versus","during","about"}
        extra_excludes = {"additionally","explanation","provided","evidence","therefore","insufficient","based","solely","given","cannot","explanation:"}
        proper = re.findall(r'([A-Z][a-zA-Z-]+(?:\s[A-Z][a-zA-Z-]+)*)', text)
        cleaned = []
        for m in proper:
            m2 = re.sub(r'^(Are|Is|Was|Were|Do|Does|Did|Can|Could|Would|Should|Which|What|Who|Whom|Whose|Where|When|Why|How)\s+', '', m)
...
class PromptBuilder:
    @staticmethod
    def build_prompt(question: str, retrieved_docs: List[Dict], model_name: str = "Qwen/Qwen2.5-7B-Instruct", reasoning_steps: Optional[List[str]] = None, forbid_unknown: bool = False) -> str:
        context = "\n".join([f"[Doc{d['doc_id']} score={d['score']}] {d['content']}" for d in retrieved_docs if d.get('content') and d.get('content') != 'No relevant information'])
        question_lower = question.lower()
        specific_instruction = ""
        is_yes_no = bool(re.search(r'^\s*(?:are|is|do|does|did|can)\b', question_lower)) or (" both " in question_lower) or (" either " in question_lower)
        if is_yes_no:
    ...
    @staticmethod
    def build_prompt_with_facts(question: str, retrieved_docs: List[Dict], model_name: str = "Qwen/Qwen2.5-7B-Instruct", reasoning_steps: Optional[List[str]] = None, forbid_unknown: bool = False, known_facts: Optional[List[str]] = None) -> str:
        context = "\n".join([f"[Doc{d['doc_id']} score={d['score']}] {d['content']}" for d in retrieved_docs if d.get('content') and d.get('content') != 'No relevant information'])
        facts_section = ""
        if known_facts:
...
class MultiTurnManager:
    ...
    def rewrite_query(self, session_id: str, cur_query: str) -> str:
        ...
        last_entity = last_turn.get("main_entity", "") if last_turn else ""
        last_question_entity = last_turn.get("question_entity", "") if last_turn else ""
        last_answer_entity = last_turn.get("answer_entity", "") if last_turn else ""
        ...
        if any(p in f" {cur_query_lower} " for p in (pronoun_triggers + possessive_triggers)):
            ...
            if non_person_target and re.match(r"^[A-Z][a-zA-Z]+(?:\s[A-Z][a-zA-Z]+)*$", non_person_target) and not re.search(r"\b(?:city|country|state|place|region|town|area|village|province|island|continent|location)\b", non_person_target.lower()):
                non_person_target = ""
            if not non_person_target:
                non_person_target = human_target
            
            if human_target:
                rewritten = re.sub(rf"\b(his|her|their)\s+{relation_terms}\b", lambda m: f"{human_target}'s {m.group(2)}", rewritten, flags=re.IGNORECASE)
                if re.search(rf"\b{relation_terms}\b", rewritten, flags=re.IGNORECASE):
                    ...
                    if last_rel:
                        rewritten = re.sub(r"\b(she|her|he|him|they|them)\b", f"{human_target}'s {last_rel}", rewritten, flags=re.IGNORECASE)
            if human_target:
...
class LLMHandler(BaseGenerator):
    # 修改：默认使用 Qwen/Qwen2.5-7B-Instruct
    ...
    def _call_llm(self, messages: List[Dict], model_name: Optional[str] = None, temperature: float = 0.1, max_tokens: int = 512) -> str:
        payload = {
    ...
    def generate(self, query: str, retrieved_docs: List[Dict], max_new_tokens: int = 256, reasoning_steps: Optional[List[str]] = None, forbid_unknown: bool = False, known_facts: Optional[List[str]] = None) -> str:
        if known_facts:
            prompt = PromptBuilder.build_prompt_with_facts(query, retrieved_docs, self.model_name, reasoning_steps, forbid_unknown, known_facts)
        else:
    ...
    def plan_reasoning_steps(self, question: str) -> List[str]:
        ...
        steps.append("Synthesize a final answer that is concise. Prefer a supported answer over 'I don't know' whenever any evidence mentions the queried entities; only return 'I don't know' if none of the evidence is relevant.")
        return steps

    def plan_next_step(self, original_question: str, history: List[Dict]) -> Dict:
        q = original_question.strip()
        q_lower = q.lower()
        is_yes_no_original = bool(re.match(r"^\s*(?:is|are|was|were|do|does|did|can|could|would|should|has|have|had|will|shall)\b", q_lower))
        history_lines = []
        ...
        if action == "ask":
            ...
            if not is_yes_no_original and re.match(r"^(?:is|are|was|were|do|does|did|can|could|would|should|has|have|had|will|shall)\b", sub_q.lower()):
                try:
                    alt_subqs = self.decompose_complex_query(q)
                    alt_subqs = [s for s in alt_subqs if not re.match(r"^(?:is|are|was|were|do|does|did|can|could|would|should|has|have|had|will|shall)\b", s.lower())]
                    if alt_subqs:
                        sub_q = alt_subqs[0]
                except Exception:
                    pass
    ...
    def _should_decompose(self, question: str) -> bool:
        q_lower = question.lower()
        triggers = [" in which ", " of which ", " who was ", " who were ", " which constituency ", " acted as ", " home of ", " music director ", " first elected ", " frontman of ", " born first", " written by ", " features the story ", " shot a man ", " and ", " or ", " both ", " either ", " vs ", " versus ", " compared", " compare ", " are both ", " which is ", " which was ", " who is ", " that "]
    ...
    def _rule_based_decompose(self, question: str) -> Optional[List[str]]:
        ...
        if m_or:
            ...
            if re.match(r"(was|is|did|does|do|can|could|should|would)", rest, re.IGNORECASE):
                rest = re.sub(r"^(was|is|did|does|do|can|could|should|would)\s+", "", rest, flags=re.IGNORECASE)
    ...
    def decompose_complex_query(self, question: str) -> List[str]:
        ...
        try:
            messages = [{"role": "system", "content": "You are a helpful assistant that outputs only JSON."}, {"role": "user", "content": decompose_prompt}]
    ...
    def self_check_answer(self, question: str, draft_answer: str, retrieved_docs: List[Dict]) -> str:
        ...
        messages = [{"role": "system", "content": "You are a meticulous fact-checking agent."}, {"role": "user", "content": verify_prompt}]
        ...
        if revise_match:
            ...
            new_answer = self.generate(refined_query, new_pruned, reasoning_steps=reasoning_steps, forbid_unknown=has_any_coverage)
    ...
    def _sanitize_sub_questions(self, original_question: str, sub_questions: List[str]) -> List[str]:
        ...
        if not is_yes_no_original:
            cleaned = [s for s in cleaned if not re.match(r"^(?:is|are|was|were|do|does|did|can|could|would|should|has|have|had|will|shall)\b", s.lower())]
...
class RAGPipelineManager:
    ...
    def _run_iterative_planner(self, session_id: str, original_question: str, retrieval_method):
        ...
        final_answer = self.llm_handler.generate(original_question, pruned_docs, reasoning_steps=final_reasoning, forbid_unknown=has_any_coverage, known_facts=known_facts)
    ...
    def run(self, session_id: str, question: str, retrieval_method: str = "dense") -> Dict:
        ...
        print(f"Rewritten Query: {rewritten_query} | Method: {retrieval_method}")
        try:
        ...
        if retriever is None:
            ...
            draft_answer = self.llm_handler.generate(rewritten_query, [], reasoning_steps=reasoning_steps, forbid_unknown=False)
            final_answer = self.llm_handler._apply_type_constraints(rewritten_query, draft_answer.splitlines()[0].strip())
            return {
                "final_answer": final_answer,
        ...
        print("Pruned Documents (top 3 content snippets):\n" + "\n".join([f"[Doc{d['doc_id']}] {d['content'][:150]}..." for d in pruned_docs[:3]]))
        
        reasoning_steps = self.llm_handler.plan_reasoning_steps(rewritten_query)
        has_any_coverage = any(c > 0 for c in coverage.values())
        draft_answer = self.llm_handler.generate(rewritten_query, pruned_docs, reasoning_steps=reasoning_steps, forbid_unknown=has_any_coverage)
...
if __name__ == '__main__':
    # 配置输入输出文件列表
    # 假设 retrieval.py 下载的文件在 data/ 目录下
    tasks = [
        {
    ...
    for task in tasks:
        ...
        if not os.path.exists(input_file):
            print(f"\n⚠️  Skipping {description}: File {input_file} not found.")
            continue
